  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 pb-20">
      ...
      <main className="px-4 py-6 max-w-lg mx-auto space-y-6">
        ...
        >
          <Card className="bg-gradient-to-r from-primary-50 to-secondary-50 dark:from-primary-900/30 dark:to-secondary-900/30 border border-primary-100 dark:border-primary-800">
        ...
        <motion.div
          ...
          className="flex bg-white dark:bg-gray-800 rounded-xl p-1 shadow-sm"
        >
          {TABS.map((tab) => (
            <button
              key={tab.id}
    ...
    >
      ...
      <p
        className={`text-xs font-semibold text-center truncate max-w-[70px] ${
          player?.isCurrentUser ? 'text-primary-600 dark:text-primary-400' : 'text-gray-700 dark:text-gray-200'
        }`}
      >
        {player?.name || '---'}
      </p>
...
function LeaderboardRow({ rank, player, isLast }) {
  return (
    <div
      ...
      } ${player?.isCurrentUser ? 'bg-primary-50 dark:bg-primary-900/30' : ''}`}
    >
      {/* Rank */}
      <div
      ...
      <div
        className={`w-10 h-10 rounded-full ${
          player?.isCurrentUser ? 'bg-primary-500' : 'bg-gray-200 dark:bg-gray-600'
        } flex items-center justify-center`}
      >
        <span
          className={`font-bold ${
            player?.isCurrentUser ? 'text-white' : 'text-gray-500 dark:text-gray-300'
          }`}
        >
          {player?.name?.charAt(0) || '?'}
        </span>
      ...
      <div className="flex-1 min-w-0">
        <p
          className={`font-semibold truncate ${
            player?.isCurrentUser ? 'text-primary-700 dark:text-primary-300' : 'text-gray-800 dark:text-gray-100'
          }`}
        >
          {player?.name || '---'}
        </p>
